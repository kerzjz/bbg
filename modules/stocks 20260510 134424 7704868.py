...
def req(path, **kw):
    """
    兼容原 req 函数签名。
    底层改为新浪财经/腾讯财经/AkShare 免费接口。
    """
    code = kw.get("code", "")
    region = kw.get("region", "US")
    cat = kw.get("type", "stock").lower()

    # get_stock_chart 没有传 type，从 path 推断品类
    ...
    # ==================== 外汇 ====================
        ...
        try:
            ...
            base = code[:3].upper() if len(code) >= 3 else code.upper()
            cny_col = forex_map.get(base, base)

            rate = latest.get(cny_col, "N/A")
            try:
                prev = df.iloc[-2].get(cny_col, rate)
                ch = round(float(rate) - float(prev), 4)
                chp = round(ch / float(prev) * 100, 2) if float(prev) != 0 else 0
            except Exception:
                ch, chp = 0, 0
